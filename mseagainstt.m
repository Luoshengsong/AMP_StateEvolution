function [ ampsim_tau2, ampsim_mse ] = mseagainstt(N, K, u_g, v_g, MC, M, sigmaw2, inter_max)
...
% %     A = A_unnormalized*S; % normalized columns of A
    ...
    x(ind(1:K)) = u_g + sqrt(v_g) * randn(K,1);           % create a Gaussian K-sparse source
...
%     x(ind(1:K)) = sign(randn(K,1));   % create a +-1 K-sparse source
    ...
    [~, se_tau2, se_mse] = amp(A, y, K/N, u_g, v_g, x, inter_max);
